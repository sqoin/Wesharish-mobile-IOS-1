...
class AboutViewController: UIViewController {
    ...
    private func setActions() {
        blog.button.tap = strongify(self) { myself in
            myself.presentURL(string: "https://www.linkedin.com/company/sqoin/")
        }
        twitter.button.tap = strongify(self) { myself in
            myself.presentURL(string: "https://twitter.com/SQOIN1")
        }
        reddit.button.tap = strongify(self) { myself in
            myself.presentURL(string: "https://www.youtube.com/channel/UCQdDjelQcsXp32bpsupymBw")
        }
