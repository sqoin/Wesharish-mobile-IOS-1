...
class HomeScreenCell: UITableViewCell, Subscriber {
    ...
    func set(viewModel: HomeScreenAssetViewModel) {
        ...
        icon.tintColor = UIColor(red: 0.85, green: 0.65, blue: 0.13, alpha: 1.00)
        iconContainer.layer.cornerRadius = (iconContainer.frame.width / 2)
        currencyName.text = "WeSharish"
        currencyName.textColor = viewModel.currency.isSupported ? .white : .disabledWhiteText
        price.text = " "
        var start = viewModel.tokenBalance.index(viewModel.tokenBalance.startIndex, offsetBy:0)
        var end = viewModel.tokenBalance.index(viewModel.tokenBalance.endIndex, offsetBy: 0)
        var range = start..<end
        if viewModel.tokenBalance.count>3{
         start = viewModel.tokenBalance.index(viewModel.tokenBalance.startIndex, offsetBy:0)
         end = viewModel.tokenBalance.index(viewModel.tokenBalance.endIndex, offsetBy: -3)
         range = start..<end
        }
        fiatBalance.text = String(viewModel.tokenBalance[range])+"TND"
        fiatBalance.textColor = viewModel.currency.isSupported ? .white : .disabledWhiteText
        
        print("azezaezaezaezaeazeaeazeazeazeazeazeazeazeaze")
        debugPrint(viewModel.tokenBalance.count)
        print("aaaaaaaaaaaaazazazaazazazazazazazazazazazaza")
        tokenBalance.text = String(viewModel.tokenBalance[range])+"WSC"
